...
async def finish_reg(message: types.Message, state: FSMContext):
    ...
    p = re.compile(r'(ук([раинаеойцы]{3,}))|(россия)|(рашка)|(раися)|(путин[уа]*)|(навальн[ыйому]+)|(укропы)|(\sлох)|(\sсука)|(ебал)|(ебать)|(ху[йея])|(хер)|(пизд)|(пидор)|(пидар)|(гандон)|(твар[иь])|(\sненави)|(суицид)|(гнида)|(сос[ауёеи])|(\sсекс)|(\sпереспать)|(\sтрахну)|(ебу)|(еб\s)|(ёб\s)|(войду)|(куни)|(член)|(пенис)|(сиськи)|(выбор[ыа])|(лизать)|(дурак)|(идиот)|(жирный)|(урод)|(говно)|(дерьмо)|(параша)|(хохл)|(бля)|(тво[яюе] мат)|(твой батя)|(иди\s)')
